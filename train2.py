...
​
        ...
        ### FORSE VA MESSO UN SOFTMAX
        ##encoder_froze = model_froze(src, valid_lengths)
        ##last_probs_froze=encoder_froze[encoder_froze.size(0)-1].to(device)
...
​
        ...
        for i, enc in enumerate(encoder):
            if flag_distill==True and i < encoder.size(0)-1: 
                loss_distill += mse_loss(enc.permute(1,0,2),last_probs).to(device) ##Fa MSE su tutte le uscite tranne l'ultimo
        ...
        ##Questo e' sbagliato perche' il for sopra fa gia' tutte le uscite.
        if i % 300 ==0:
            if bpe_flag==True:
        ...
        loss = loss_layer
        if flag_distill==True:
            loss = loss_layer + loss_distill ###ATTENZIONE CHE ORA STAI SOMMANDO MSE CON CTC E VA CONTROLLATA LA SCALA
